...
def get_chart_data(apt_name: str = Query(...), months: int = Query(12), area_type: str = Query("84"), exclude_direct: bool = Query(False)):
    ...
    cur.execute("""
        SELECT built_year, built_str, units_str, type_info 
        FROM apt_meta_master 
        WHERE norm_name = ? OR apt_name = ?
    """, (clean_k, pure_name))
                  ...
                  <span class="help-tooltip-trigger">?
                    <div class="help-tooltip-box">
                      ...
                      <div class="tooltip-def">
                        <strong>정의:</strong> 해당 연도 실거래 평당가의 <strong>변동계수(CV% = 표준편차/평균가)</strong>입니다.
                      </div>
                    </div>
                  </span>
                </th>
      ...
      <th class="metric-col">
        ...
        <span class="help-tooltip-trigger">?
          <div class="help-tooltip-box">
            ...
            <div class="tooltip-def">
              <strong>누적 평균가:</strong> 선택하신 <strong>${periodDisplayStr} 동안 발생한 모든 실거래가의 단순 산술평균</strong>입니다.
            </div>
          </div>
        </span>
      </th>`;
      ...
      for (let i = 0; i < currentSlotCount; i++) {
        ...
        try {
          const excludeDirect = document.getElementById('excludeDirectChk')?.checked ? 'true' : 'false';
            const res = await fetch(`/api/chart-data?apt_name=${encodeURIComponent(aptName)}&months=${months}&area_type=${currentAreaMode}&exclude_direct=${excludeDirect}`);
